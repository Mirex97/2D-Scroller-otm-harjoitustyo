...
public class Conductor extends Animate {
    ...
    public Conductor(Map map, Area area, double x, double y) {
        super(map, area);
        this.area = area;
        this.setXY(x, y);
        setImage("/characters/conductor/Conductor.png");
        setCollision(x, y);
        middleX = image.getWidth() / 2;
        middleY = image.getHeight() / 2;
        this.setWalkingSpeed(5);
    ...
    public void move(Player player, Area instructions, long l) {
        ...
        if (action == Animate.Action.JUMPING) {
            up(false);
        }

        if (player.getMiddleX() < this.x) {
            left();
        }
        if (player.getMiddleX() > (this.x + this.width)) {
            right();
        }
